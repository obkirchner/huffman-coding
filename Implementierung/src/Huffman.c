...
int coded_index = 0;
  ...
  //print the Frequency array in a readable form
  printf("1: Print the frequency table... \n");
  printf("Characters | Frequency\n");
  printf("-----------+----------\n");

  for (size_t i = 0; i < 256; i++){
    if (i == 32 && frequency[i] > 0) {
        printf("       | %6d\n", frequency[i]);
        continue;
    }
    if (frequency[i] > 0){
  ...
  printf("2: Printing the created dictionary... \n");
  printf("Symbol | Kodierung\n");
  printf("-------+----------\n");
  ...
  printf("3: Encoding the given String ...\n");
  ...
  for (size_t i = 0; i < len; i++){
    for (size_t j = 0; j < 256; j++){
      ...
      if (dict[j] != 0 && data[i] == (char)j){
        printf("%s ", dict[j]);
        //strncat(coded, dict[j],strlen(dict[j]));
        strcpy(coded, dict[j]);
